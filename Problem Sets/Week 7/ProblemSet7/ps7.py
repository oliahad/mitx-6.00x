...
class RectangularRoom(Position):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        """
        self.pos=pos
        if not(math.floor(self.pos.getX()),math.floor(self.pos.getY())) in self.cleaned.keys():
            self.cleaned[(math.floor(self.pos.getX()),math.floor(self.pos.getY()))]=True
    ...
    def isTileCleaned(self, m, n):
        ...
        self.m=m
        self.n=n
        if (self.m,self.n) in self.cleaned.keys():
            return True
        return False
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        self.pos=pos
        return (0 <= self.pos.getX() < self.width) and (0 <= self.pos.getY() < self.height)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                RandomWalkRobot)
    """
    anim = ps7_visualize.RobotVisualization(num_robots, width, height,0.2) #anim = ps7_visualize.RobotVisualization(num_robots, width, height,delay)
    time=0
    for i in range(num_trials):
        ...
        while AreaCleaned<min_coverage:
            ...
            AreaCleaned=float(room.getNumCleanedTiles())/room.getNumTiles()
    anim.done()
    return math.floor(time/num_trials)
...
#
#       (... your call here ...)
#showPlot1('Variation of Cleaning time with number of Robots','Number of Robots used','Time taken to clean')
...
#
#       (... your call here ...)
#showPlot2('Comparing Cleaning time with aspect ratio','Aspect-Ratio(Width/Height)','Time taken by the Robot')
#
